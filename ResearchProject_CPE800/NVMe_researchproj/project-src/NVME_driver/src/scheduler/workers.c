...
static int8_t IO_send(Async_transport_ctx *transport_ctx, Nvmeq_context_t *IOctx, uint16_t new_cid)
{
    ...
    *((volatile uint32_t*) (uintptr_t) IOctx->sq_tdbl) = (uint32_t) IOctx->sq_tail;
    ...
    if(transport_ctx->update_requests(transport_ctx, STATE_PENDING, STATUS_PENDING, new_cid) == EXIT_FAILURE) {
        return -1;
    }
    return 0;
...
static int8_t IO_receive(Async_transport_ctx *tctx, Nvmeq_context_t *IOctx)
{
    ...
    while (1) {
        ...
        MEM_FENCE(r, rw); // Acquire barrier

        uint16_t cid = entry->cid;
        uint8_t final_status = (entry->dw3.sf != 0) ? STATUS_ERR_NVME : STATUS_SUCCESS;
        
        if (tctx->update_requests(tctx, STATE_DONE, final_status, cid) == EXIT_FAILURE) {
            return -1;
        }
    ...
    while (self->running) {
        ...
        */
        ...
        if (task.cid == 0xFFFFFFFF 
            && task.absolute_deadline == 0xFFFFFFFFFFFFFFFF
                && task.timestamp_start == 0xFFFFFFFFFFFFFFFF) {
            continue;
        }
        ...
        if (current_time > task.absolute_deadline) {
            /* Update the request */
            if(self->tctx.update_requests(&self->tctx, 
                        STATE_DONE, 
                        STATUS_DEADLINE_PASSED, 
                        task.cid) == EXIT_FAILURE) {
