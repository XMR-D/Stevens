...
static int PushToFileList(char *filename, struct stat *sb, int ishidden, FileList *list) 
{
...
int FileListInsert(char *filename, FileList *filelist, FileList *reclist) 
{
    ...
    if (S_ISDIR(sb.st_mode))
        PushToFileList(strdup(filename), &sb, ishidden, reclist);

    PushToFileList(strdup(filename), &sb, ishidden, filelist);
